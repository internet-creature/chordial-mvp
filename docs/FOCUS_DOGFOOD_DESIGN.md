...
- ▸ (and Enter, and the row's start) on a task that is owed its scope opens the form
  rather than starting - the person is never one click from a blind block; a task that
  already has a scope, or skipped it today, starts straight away.
- "just start" (or "start" with a blank line) starts without a scope and remembers the skip for this task for today
...
migration: one alembic revision adding the nullable columns (`next_action`, `set_aside_on`,
...
| 4 | scope + set aside | (was 3) — **BUILT 2026-09-08**; review round: parking the running task aborts unless the sidecar confirms the clock stopped (a swallowed pause error would have parked a task whose timer kept counting); the label is split against the task's canonical title, never on the first `": "` (titles are unrestricted); an open row shows the whole title; the head is now a small deer at the top-left with an always-present bubble (latest line, else a factual day line from `app/src/lib/bubble.ts`) - the big centred emoji + "loafing nearby" caption is gone (dogfood: it detracted). follow-on, not this round: a **custom block length** beside 10·25·50 for edge cases (willowden-first). on `dogfood/scope-and-set-aside`: the scoping form (▸ / start / Enter on an unscoped task opens it instead of running blind; "start" PATCHes the line then runs `"<title>: <scope>"`; "just start" or a blank line runs unscoped and is remembered per task per local day in `localStorage`), click-to-edit on an existing scope (same PATCH, never starts a clock), the label split (den: title over scope; bar: `title · scope`), "set aside for today" on every expanded row (the running one pauses first, then patches), the parked section below the wins with tomorrow / bring back / let it go, Home's greyed "set aside" group, and `target_minutes` on `session.started`. copy lives in `app/src/lib/scope.ts` (`SCOPE_COPY`) for the voice pass. smoke-verified in the browser pane against a scratch sidecar + a mocked server |
| 5 | vel's day | (was 4) — **BUILT 2026-09-09** on `dogfood/vels-day`, server-only: `src/services/focus_day.py` (`snapshot` / `render` / `digest` / `checkin_posture` / `breakdown_flags`) reads the user's applied device events for one local day plus the workspace; the digest rides every daily turn (ticks AND user turns) after the agenda digest, and the morning brief gets yesterday's digest as a prelude (§13.3); scheduled ticks other than the brief carry `checkin_posture` ∈ mid_block / stuck / untouched / between / wrapped / quiet_day + `posture_detail`, rendered by `_scheduled_instruction` (the plain check-in keeps its bytes when the snapshot fails); the plan carries `presence` + `idle_minutes` into the stimulus and `enrich` appends the §5.2 line; `needs_breakdown` on today-payload rows. built-as-decided: `between` applies whenever runs are banked and no clock runs (the idle minutes ride in the detail - the 20-min threshold isn't a separate posture); an evening wraps only if the day banked or finished something (nothing to settle otherwise - the plan still invites); the initiation-failure signal is the server-observable version (still unscoped after two short starts - the skip lives in localStorage); `first_block` for the untouched posture skips flagged tasks; a `session.started` older than yesterday's start is not a running clock. mid_block's silence relies on the tick's `response="optional"` line - verified that an empty reply is accepted as silence by the engine. **sol's round:** the sidecar emits `session.frozen` when an open rewind question freezes a run (a started with no transition after it read as mid-block all afternoon; the digest now says the clock stopped, unbanked); set aside is read from a new `task_set_asides` ledger (migration `c3a9f1e7d2b5`, backfilled from the stamps) because tomorrow / bring back clear the stamp and yesterday's digest must keep what was parked; "on the list two days" also requires the task to have existed by the start of today and not been parked yesterday; a tick takes ONE snapshot and renders posture and digest from it |
| 6 | ask pip | (was 5) |
